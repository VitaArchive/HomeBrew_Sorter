...
static int check(Title *HBlist, char* dir,int HBfound, int flag) {
	...
	while (1) {
		...
		if (FIO_S_ISDIR(oneDir.d_stat.st_mode)) {
			...
			if (eboot_exists(fullName)) {
				...
				HBlist[HBfound].type = TITLE_EBOOT;
...
// Browser Mode
int saveTitlesListBM(Title *HBlist, int HBcount) {
	int i = 0;
	char temp[262];
