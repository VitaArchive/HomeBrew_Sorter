...
static void decompress_dax0(void* src, u32 src_len, void* dst, u32 dst_len, u32 topbit){
	...
	sctrlDeflateDecompress(dst, src, dst_len);
}
...
int iso_alloc(u32 com_size) {
	...
	if (is_compressed) {
		...
		g_ciso_dec_buf = malloc(com_size+64);
		...
		g_ciso_block_buf = malloc(com_size+64);
		...
		g_cso_idx_cache = malloc((g_cso_idx_cache_num * 4) + 64);
		if (g_cso_idx_cache == NULL) {
			return -4;
		}
	}
	// allocate iso sector buffer
	g_sector_buffer = malloc(ISO_SECTOR_SIZE);
...
void iso_free() {
	#ifdef __USE_USER_ALLOC
	if (is_compressed) {
		free(g_ciso_dec_buf_alloc_ptr);
		free(g_ciso_block_buf_alloc_ptr);
		free(g_cso_idx_cache);
	}
	free(g_sector_buffer);
	#else
	if (is_compressed) {
